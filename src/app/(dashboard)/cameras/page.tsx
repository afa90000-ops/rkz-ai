...
import { Camera, Plus, Settings, Play, Trash2, WifiOff, X } from 'lucide-react'
...
export default function CamerasPage() {
  ...
  async function handleAdd() {
    if (!form.name_ar) return; setSaving(true)
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:'10px' }}>
        <div>
          ...
          <p style={{ fontSize:'12px', color:'#3d4f6e', marginTop:'2px' }}>{cameras.length} كاميرا</p>
        </div>
      ...
      {loading ? (
        <div className="grid-2">
          {[1,2,3,4].map(i=><div key={i} style={{ background:'#070d1a', border:'1px solid #1a2540', borderRadius:'16px', height:'260px' }}/>)}
        </div>
      ) : cameras.length===0 ? (
        <div style={{ background:'#070d1a', border:'1px solid #1a2540', borderRadius:'16px', padding:'50px', textAlign:'center' }}>
          ...
          <div style={{ fontSize:'14px', color:'#3d4f6e' }}>لا توجد كاميرات — اضغط "إضافة"</div>
        </div>
      ) : (
        <div className="grid-2">
                ...
                <div style={{ position:'relative', aspectRatio:'16/9', background:'#040812', overflow:'hidden' }}>
                  ...
                  {camera.status==='online' ? (
                    ...
                    <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center' }}>
                      <WifiOff size={30} color="rgba(255,51,102,.4)"/>
                    </div>
                  )}
